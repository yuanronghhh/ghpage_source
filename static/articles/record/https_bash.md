...
## 原理
在数学中,有一类函数,正向计算很容易, 但反向计算很困难,只有多给出后门,才能计算出结果,
所以人们利用这种原理,设计了非对称加密算法

非对称加密, 就是利用数学原理, 使用算法生成在加密软件中生成的两个钥匙, 
其中一个用于解密,另一个用于加密. 它们生成后,具有以下这些特点:
1. 公钥和私钥地位平等,是算法中的两个因子,均可用于加密和解密, 一般公钥公开,私钥不公开.
...
### 疑问
说明流程之前, 先提个问题, 假设有`A`,`B`两个人, 如果`B`要寄信,但不希望邮局`C`知晓内容,如何做到?
...
上面例子, 在计算机中, 就是非对称加密, `箱子`就如`公钥`,`钥匙`就如`私钥`, 
每个人能免费获得他人的箱子, 但无法得到钥匙就无从知晓内容.

### 计算机中流程
像上面这样,伪造可能花费较多成本, 但计算机中, 密钥对由软件生成
上面的流程对应到计算机中, 便是这样的:
...
明文传输下, 中间人`C`也可以生成公钥和私钥对的, `C`可以将`A公钥`替换成自己的`C公钥`,
同样,也可以将`B`传给`A`的公钥改成自己的`C公钥`, 达到中间监听信息的目的.这是常见的
中间人攻击问题.
...
- `MD5`算法: 这是一个哈希算法, 通过数据填充,分组循环变换,拼接输出,得到一个不可字符串.
