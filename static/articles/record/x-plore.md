{
  ...
  "update_at": "2019-02-17T00:00:00"
}
# x-plore在安卓平台依然强大

## 前言

从拥有第一部智能手机到今天，自己已经用`x-plore`有一定的年头了吧，但今天，读了一下官网的文档，研究了一下，发现这么小的软件居然有这么多的功能，很是诧异，所以在此也希望更多的人能全面地认识一下`x-plore`这个软件，而不要因为复制文件不知道到去哪里了就放弃了，因为，它真的很强大。

> 请注意：
> 1. 对于普通用户，仅需要阅读一下基本篇即可。
> 2. 请善用搜索(ctrl+f)找到需要的部分阅读，不感兴趣的部分请自行跳过。
> 3. 如果您发现文章中出现了错误，请直接[@me](mailto:635044633@qq.com), 我会非常感谢您。
> 4. 本介绍的`x-plore`版本是安卓`v4.10.01`，可能和你的界面上有些许出入，但整体功能不影响。
...
> 此时`文件a`可以是多选多个文件，在勾选`移动模式`的时候，点击文件名可以重命名为新的文件。
...
2. 解压zip或rar文件。
`x-plore`可以浏览和解压单个`rar`/`zip`格式的压缩包，但不支持分卷压缩的包进行解压。
...
第三步，如果另一部手机(手机B)同样装有`x-plore`，则可以点击`无线文件共享`-`添加设备`-`扫描`，扫描比较慢，可以直接点击停止就可以立即看到出现了一部手机(手机A)，接着就可以在两部手机直接操作文件了。 如果另一部手机(手机B)没有安装`x-plore`，则可以直接使用`浏览器`输入`http://192.168.43.1:1111`(请注意需要添加`http://`)，访问即可出现x-plore的`Web访问`功能， 同样可以进行下载，上传操作。
...
> 1. 如果对于希望修改音频的**专辑名**，**歌手**,**封面**等信息，可以点击详情，第三个选项卡`ID3`就是音频信息，点击右边的`笔型图标`就可以修改了。
> 2. 目前不支持歌词的播放。
...
### 指纹传感器
指纹传感器是新功能，用于`x-plore`应用启动时上锁功能，和程序密码一样, 不过可以使用指纹解锁。
...
`x-plore`安卓版本历史：
1.50 &emsp; 初始Android版本
1.56 &emsp; Android1.x最后一个版本
2.00 &emsp; (Dec 09 2011) &emsp; 完成Android原生应用的重写(工作)
2.03 &emsp; (Dec 14 2011) &emsp; 允许安装到SD Card, 修复了一些崩溃，添加了斯洛伐克语，捷克语，俄语支持，支持隐藏文件夹
2.10 &emsp; (Dec 21 2011) &emsp; 支持局域网LAN(SMB)共享,崩溃修复，添加德语
2.11 &emsp; (Dec 27 2011) &emsp; 添加RAR读取功能，添加土耳其语
2.20 &emsp; (Dec 30 2011) &emsp; 添加数据库*.db文件格式的读取功能，添加波兰语，日本语，中文，支持文件日期排序功能
2.21 &emsp; (Jan 7 2012) &emsp; 添加法语, 修复"使用市场版"(use Market version)提示bug
2.30 &emsp; (Jan 11 2012) &emsp; 添加Picasa图片相册功能, 提高缩略图创建速度
2.40 &emsp; (Jan 19 2012) &emsp; 添加FTP/FTPS服务连接功能，添加韩语支持，增强图片查看器功能
2.45 &emsp; (Jan 25 2012) &emsp; 支持从其他地方分享文件：支持本地，网络，zip格式。更多系统集成：添加文件夹快捷方式到主屏幕，使用x-plore打开Zip，Rar文件，选择文件(比如：从邮件中选择附件)
2.47 &emsp; (Jan 31 2012) &emsp; 新建文本文件，设置文件权限(在root目录下的文件)，桌面快捷方式也适用于文件，配置选项：默认以ZIP方式打开APK，以滑动替代切换窗格，修复Rar解压bug
2.48 &emsp; (Feb 3 2012) &emsp; 新增按钮上/下 翻页，少量修复
2.50 &emsp; (Feb 22 2012) &emsp; 启动(设置)密码，使用情况统计，修正：root系统/搜索系统/外置存储卡/Picasa相册/流传输
2.52 &emsp; (Mar 6 2012) &emsp; 添加意大利语，支持在root文件夹下编辑text文件，选项：全屏图片查看器，修复：`/System`文件夹下root一些功能
2.53 &emsp; (Mar 13 2012) &emsp; 从网络路径打开任意一些文件(自动下载到临时目录)，可配置列表高度，修复：局域网(LAN)服务下的权限问题
2.60 &emsp; (Apr 6 2012) &emsp; 网络存储(Web storage)：Box.net，Sugarsync。自动备份设置
2.65 &emsp; (Apr 12 2012) &emsp; `Dropbox` 支持，为 `Android 3.0+`增强主题，修复文本编辑器
2.70 &emsp; (May 1 2012) &emsp; `Google Drive`网盘支持
2.81 &emsp; (May 16 2012) &emsp; `SkyDrive`网盘支持, 添加巴西，葡萄牙语
3.00 &emsp; (Jul 19 2012) &emsp; `Webdav` + `Yandex`云端存储，添加越南语
