...
![](dist/images/window.gif)
我们可以看到，`x-plore`有左右2个窗口，分别代表我们`源`(source)和`目标`(destination)的路径，
我们需要先选择`源`，然后确定`目标`，才能够利用`操作面板`操作。   

比如，我们需要移动文件，则需要先在左边勾选需要重命名的`文件a`，然后左划屏幕到右边选择需要移动的目的`位置b`，最后再滑动点击右侧中间的`复制`按钮，勾选`移动模式`，就可以将`文件a`移动到`b位置`。
...
- 收藏夹
如果操作次数比较频繁，可以将文件夹标记起来，标记过的文件夹将在根列表面板下显示。并且无法隐藏。
...
### 网盘
可以将文件管理器作为网盘客户端连接，像`OneDrive`这样的会进行授权才可以使用，国内网盘基本没戏。

### 应用程序管理
这里默认显示本机安装的第三方软件，可以通过长按，勾选`显示系统应用程序`，就可以显示所有软件了，这里如果对软件删除，则会提示卸载操作。在共享时，这个地方可以方便地交换双方的软件进行安装。

### DLNA
数字生活网络联盟，可以对多媒体跨平台投放，比如可以将手机音乐投放到数字电视上面播放，具体笔者没有尝试，不做详细说明。

### Send Anywhere
这个是新功能，也是分享文件，不过主要方便分享到send anywhere这个平台上面，可以通过扫码的方式(要先装一个二维码扫描程序)获取的方式获取文件。
我想吐槽的是，这个官方客户端比较大，还不如`x-plore`方便。
...
#### 文本编辑器
文本支持`utf-8`、`utf-16`、`us-ascii`三种编码，亦可搜索文本。

#### 音乐播放器
如果在`设置`里面使用自带查看器的音频，则点击`mp3`文件后会直接按顺序播放该文件夹下的**所有音乐文件**。
...
2. `系统文件夹`：如果你将软件安装到SDCard，那么当你拔下内存卡插到电脑上时，可以才看到这个`.android`的文件夹，而在Android手机里面因为系统保护，是看不到的。这也是我们内存卡容量不知不觉减小的原因，不过现在手机都一体化了, 很难看到了。
